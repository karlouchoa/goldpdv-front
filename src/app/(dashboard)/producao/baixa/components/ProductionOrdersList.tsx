...
import { listProductionOrdersInProduction } from "@/modules/production/services/productionService";
...
export function ProductionOrdersList({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const result = await listProductionOrdersInProduction(session);
        if (cancelled) return;
        setOrders(result);
      } catch (err) {
        if (cancelled) return;
        setError(
          err instanceof Error ? err.message : "Falha ao carregar OPs em producao.",
        );
      } finally {
  ...
  return (
    <SectionCard
      title="OPs em producao"
      description="Selecione uma OP em producao para concluir."
      action={
    ...
    >
      {loading ? (
        ...
        <p className="text-sm text-slate-500">
          Nenhuma OP em producao encontrada.
        </p>
      ) : (
